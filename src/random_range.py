...
def generate_random_numbers(count, min_value, max_value, use_sample=False):
    ...
    if use_sample and (max_value - min_value + 1) < count:
        print(f"Warning: Range too small for {count} unique samples. Using duplicates allowed mode.")
        use_sample = False
...
def plot_scatter_graph(x_values, y_values, title="Scatter Plot", xlabel="X", ylabel="Y"):
    ...
    plt.figure(figsize=(10, 6))
    plt.scatter(x_values, y_values, c=y_values, cmap='viridis', s=100, alpha=0.6, edgecolors='black')
...
def plot_data(x_values, y_values, plot_type='line', title=None, xlabel="Index", ylabel="Random Value"):
    """
...
def main():
    ...
    if MATPLOTLIB_AVAILABLE:
        plot_data(numbers_a, numbers_b, 'line', 
                 title="Default Line Plot (1-12 vs Random 0-999)",
                 xlabel="Sequential Index", ylabel="Random Value")
    ...
    if MATPLOTLIB_AVAILABLE:
        plot_data(x_custom, y_custom, 'bar',
    ...
    if MATPLOTLIB_AVAILABLE:
        plot_data(x_scatter, y_scatter, 'scatter',
    ...
    if MATPLOTLIB_AVAILABLE:
        plot_data(x_unique, y_unique, 'line',
